...
Given('I visit the Events section of main menu', async ()=> {
    await sleep(2000);
...
Given('click on the CREAR EVENTO', async ()=> {
    await driver.findElement(By.name('crear_evento')).click();
    await sleep(2000);
});

When('complete the form to create the event',{timeout: 10*1000}, async ()=> {
...
Then('check the new event is created', async()=>{
    await sleep(3000);
